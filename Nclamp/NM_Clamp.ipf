...
Function ClampConfigs( )
	...
	NMConfigVar( fname, "Backup", 20, "backup ( save ) time for current Igor experiment ( minutes ). Enter 0 for no backup." )
...
Function /S ClampPathSet( pathStr )
	...
	String mssg = "Please specifiy directory where data files are to be saved."
	
	if ( strlen( pathStr ) == 0 )
		NewPath /Q/O/M=(mssg) ClampSaveDataPath
	else
		NewPath /Q/O/M=(mssg) ClampSaveDataPath pathStr
	endif
