...
Function /S WaveListFolder( folder, matchStr, separatorStr, optionsStr )
	...
	if ( DataFolderExists( folder ) == 0 )
		//NMDoAlert( "WaveListFolder error: data folder does not exist: " + folder )
		return ""
	endif
...
Function List2Wave( strList, wName )
	...
	if ( WaveExists( $wName ) == 1 )
		NMDoAlert( "Abort List2Wave: wave " + wName + " already exists." )
		return -1
	endif
...
Function EditWaves( tName, tTitle, wList )
	...
	if ( WinType( tName ) > 0 )
		...
		if ( WinType( tName ) != 2 )
			NMDoAlert( "Abort EditWaves: window already exists with that name : " + tName )
			return -1
		endif
...
Function /S RenumberWaves( from, wList )
	...
	NMDoAlert( "Alert: RenumberWaves has been deprecated." )
...
Function /S BreakWave( wName, outPrefix, npnts )
	...
	return SplitWave( wName, outPrefix, -1, npnts )
...
Function /S SplitWave( wName, outPrefix, chanNum, npnts )
	...
	for ( wcnt = 0; wcnt < nwaves; wcnt += 1 )
		...
		if ( WaveExists( $outName ) == 1 )
			...
			DoAlert 1, "Split wave error: output wave name " + outName + " is already in use. Do you want to continue?"
...
Function /S Make2DWave( wList, outname )
	...
	if ( numtype( rows ) > 0 )
		NMDoAlert( "Abort Make2DWave: waves have different number of rows." )
		return ""
	endif
...
Function /S FilterFIRwaves( alg, f1Fraction, f2Fraction, n, wList )
	String alg // "low-pass","high-pass" or "notch"
	Variable f1Fraction, f2Fraction, n // see FilterFIR
	String wList // wave list ( seperator ";" )
	...
	if ( ( numtype( f1Fraction ) > 0 ) || ( f1Fraction <= 0 ) || ( f1Fraction > 0.5) )
		NMHistory( "FilterFIRwaves abort: filter f1 frequency is out of range: " + num2str( f1Fraction ) )
		return "" // error
	endif
	
	if ( ( numtype( f2Fraction ) > 0 ) || ( f2Fraction <= 0 ) || ( f2Fraction > 0.5) )
		NMHistory( "FilterFIRwaves abort: filter f2 frequency is out of range: " + num2str( f2Fraction ) )
		return "" // error
	endif
	...
	strswitch( alg )
		...
		case "high-pass":
			...
			if ( f1Fraction >= f2Fraction )
				NMHistory( "FilterFIRwaves abort: filter f1 > f2 " )
				return "" // error
			endif
			
			if ( ( numtype( n ) > 0 ) || ( n <= 0 ) )
				NMHistory( "FilterFIRwaves abort: filter n is out of range: " + num2str( n ) )
				return "" // error
			endif
	...
	for ( wcnt = 0; wcnt < ItemsInList( wList ); wcnt += 1 )
		...
		Note $wName, "Func:FilterFIRwaves"
		
		strswitch( alg )
			case "low-pass":
				Execute "FilterFIR /LO={" + num2str( f1Fraction ) + "," + num2str( f2Fraction ) + "," + num2str( n ) + "} " + wName
				Note $wName, "FilterFIR Alg:" + alg + ";f1:" + num2str( f1Fraction ) + ";f2:" + num2str( f2Fraction ) + ";n:" + num2str( n ) + ";"
				break
			case "high-pass":
				Execute "FilterFIR /HI={" + num2str( f1Fraction ) + "," + num2str( f2Fraction ) + "," + num2str( n ) + "} " + wName
				Note $wName, "FilterFIR Alg:" + alg + ";f1:" + num2str( f1Fraction ) + ";f2:" + num2str( f2Fraction ) + ";n:" + num2str( n ) + ";"
				break
			case "notch":
				Execute "FilterFIR /NMF={" + num2str( f1Fraction ) + "," + num2str( f2Fraction ) + "} " + wName
				Note $wName, "FilterFIR Alg:" + alg + ";fc:" + num2str( f1Fraction ) + ";fw:" + num2str( f2Fraction ) + ";"
				break
			default:
				return ""
		endswitch
	...
	NMUtilityAlert( "FilterFIRwaves", badList )
...
Function /S FilterIIRwaves( alg, freqFraction, notchQ, wList )
	String alg // "low-pass","high-pass" or "notch"
	Variable freqFraction, notchQ // see Igor FilterIIR function ( freq = fHigh or fLow or fNotch )
	String wList // wave list ( seperator ";" )
	...
	if ( ( numtype( freqFraction ) > 0 ) || ( freqFraction <= 0 ) || ( freqFraction > 0.5) )
		NMHistory( "FilterIIRwaves abort: filter frequency is out of range: " + num2str( freqFraction ) )
		return "" // error
	endif
	...
	strswitch( alg )
		...
		case "notch":
			...
			if ( ( numtype( notchQ ) > 0 ) || ( notchQ <= 1 ) )
				NMHistory( "FilterIIRwaves abort: notchQ is out of range: " + num2str( notchQ ) )
				return "" // error
			endif
	...
	for ( wcnt = 0; wcnt < ItemsInList( wList ); wcnt += 1 )
		...
		Note $wName, "Func:FilterIIRwaves"
		
		strswitch( alg )
			case "low-pass":
				Execute "FilterIIR /LO=" + num2str( freqFraction ) + " " + wName
				Note $wName, "FilterIIR Alg:" + alg + ";freq:" + num2str( freqFraction ) + ";"
				break
			case "high-pass":
				Execute "FilterIIR /HI=" + num2str( freqFraction ) + " " + wName
				Note $wName, "FilterIIR Alg:" + alg + ";freq:" + num2str( freqFraction ) + ";"
				break
			case "notch":
				Execute "FilterIIR /N={" + num2str( freqFraction ) + "," + num2str( notchQ ) + "} " + wName
	...
	NMUtilityAlert( "FilterIIRwaves", badList )
...
Function /S AvgWaves( wList )
	...
	if ( numtype( dx ) != 0 )
		NMDoAlert( "AvgWaves Abort : waves do not have the same delta-x values." )
		return ""
	endif
...
Function /S AvgWavesPntByPnt( wList )
	...
	Variable dx = GetXStats( "deltax", wList )
	Variable lftx = GetXStats( "minleftx", wList )
	Variable rghtx = GetXStats( "maxrightx", wList )
	Variable npnts = 1 + ( rghtx - lftx ) / dx
	
	if ( numtype( dx ) != 0 )
		NMDoAlert( "AvgWavesPntByPnt Abort : waves do not have the same delta-x values." )
		return ""
	endif
	...
	Make /O/N=( npnts ) U_Avg
	Make /O/N=( npnts ) U_Sdv
	Make /O/N=( npnts ) U_Pnts
...
Function NMSortWave( sName, dName, method, xv, yv, nv )
	...
	NMNoteType( dName, "NMSet", xl, yl, "Func:NMSortWave" )
...
Function /S SortWaveListByCreation( wlist )
	...
	Variable wcnt, nwaves = ItemsInList( wlist )
	String wname
	
	Make /T/O/N=( nwaves ) NM_SortWavesNames = ""
	Make /D/O/N=( nwaves ) NM_SortWavesDate = inf
	
	for ( wcnt = 0 ; wcnt < nwaves ; wcnt += 1 )
	
		wname = StringFromList( wcnt, wlist )
		
		NM_SortWavesNames[ wcnt ] = wname
		
		if ( WaveExists( $wname ) == 1 )
			NM_SortWavesDate[ wcnt ] = CreationDate( $wname )
		endif
		
	endfor
	
	Sort NM_SortWavesDate, NM_SortWavesDate, NM_SortWavesNames
	
	wlist = ""
	
	for ( wcnt = 0 ; wcnt < nwaves ; wcnt += 1 )
		wlist = AddListItem( NM_SortWavesNames[ wcnt ], wlist, ";", inf )
	endfor
	
	KillWaves /Z NM_SortWavesDate, NM_SortWavesNames
